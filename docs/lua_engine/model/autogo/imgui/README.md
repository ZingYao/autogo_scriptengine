...
```lua
{
    callback = function  -- 回调函数，在每一帧中执行
}
```
...
```lua
-- 创建文本输入框
local edited, text = imgui.inputText("输入", "默认文本");
if edited then
    print("输入的文本: " .. text);
end
```
...
```lua
-- 创建复选框
local changed, checked = imgui.checkbox("启用", true);
if changed then
    print("复选框状态: " .. tostring(checked));
end
```
...
```lua
-- 创建浮点滑块
local changed, value = imgui.sliderFloat("音量", 0.5, 0.0, 1.0);
if changed then
    print("音量: " .. value);
end
```
...
```lua
-- 创建整数滑块
local changed, value = imgui.sliderInt("数量", 10, 0, 100);
if changed then
    print("数量: " .. value);
end
```
...
```lua
-- 创建颜色编辑器
local changed, r, g, b = imgui.colorEdit3("颜色", 1.0, 0.0, 0.0);
if changed then
    print("颜色: R=" .. r .. " G=" .. g .. " B=" .. b);
end
```
...
```lua
-- 创建颜色编辑器（带透明度）
local changed, r, g, b, a = imgui.colorEdit4("颜色", 1.0, 0.0, 0.0, 1.0);
if changed then
    print("颜色: R=" .. r .. " G=" .. g .. " B=" .. b .. " A=" .. a);
end
```
...
```lua
{
    ...
    btn1Text = string,   -- 第一个按钮文本（可选）
    btn2Text = string    -- 第二个按钮文本（可选）
}
```
...
```lua
{
    result = number  -- 返回用户点击的按钮索引（0: btn1, 1: btn2, -1: 取消）
}
```
...
```lua
-- 显示对话框
local result = imgui.alert("提示", "确定要继续吗？", "确定", "取消");
if result == 0 then
    print("用户点击了确定");
end
```
...
```lua
{
    message = string,  -- Toast 提示消息
    x = number,       -- 在界面上显示的 X 坐标（传递-1使用默认坐标）
    y = number,       -- 在界面上显示的 Y 坐标（传递-1使用默认坐标）
    duration = number  -- 提示显示的持续时间，单位为毫秒（传递-1使用默认2000毫秒）
}
```
...
```lua
{
    x1 = number,        -- 矩形左上角 X 坐标
    y1 = number,        -- 矩形左上角 Y 坐标
    x2 = number,        -- 矩形右下角 X 坐标
    y2 = number,        -- 矩形右下角 Y 坐标
    color = string,     -- 颜色字符串（格式："#RRGGBB" 或 "#AARRGGBB"）
...
```lua
-- 绘制红色矩形
imgui.drawRect(10, 10, 100, 100, "#FF0000", 2.0);
```
