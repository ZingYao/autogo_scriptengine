...
func (r *MethodRegistry) GenerateLuaDocumentation() string {
	...
	doc += "  通过应用包名启动应用\n\n"
	doc += "app.openSetting(packageName: string) -> boolean\n"
	doc += "  打开应用的详情页(设置页)\n\n"
	...
	doc += "-- 触摸操作 (touch)\n"
	doc += "--------------------------------------------------\n"
	doc += "motion.touchDown(x: number, y: number, fingerID: number)\n"
	doc += "  按下屏幕\n\n"
	doc += "motion.touchMove(x: number, y: number, fingerID: number)\n"
	doc += "  移动手指\n\n"
	doc += "motion.touchUp(x: number, y: number, fingerID: number)\n"
	doc += "  抬起手指\n\n"
	doc += "motion.click(x: number, y: number, fingerID: number)\n"
	doc += "  点击\n\n"
	doc += "motion.longClick(x: number, y: number, duration: number)\n"
	doc += "  长按\n\n"
	doc += "motion.swipe(x1: number, y1: number, x2: number, y2: number, duration: number)\n"
	doc += "  滑动\n\n"
	doc += "motion.swipe2(x1: number, y1: number, x2: number, y2: number, duration: number)\n"
	doc += "  滑动(两点)\n\n"
	doc += "motion.home()\n"
	doc += "  按下Home键\n\n"
	doc += "motion.back()\n"
	...
	doc += "motion.recents()\n"
	doc += "  按下最近任务键\n\n"
	doc += "motion.powerDialog()\n"
	doc += "  长按电源键\n\n"
	doc += "motion.notifications()\n"
	doc += "  下拉通知栏\n\n"
	doc += "motion.quickSettings()\n"
	doc += "  下拉快捷设置\n\n"
	doc += "motion.volumeUp()\n"
	doc += "  按下音量加键\n\n"
	doc += "motion.volumeDown()\n"
	doc += "  按下音量减键\n\n"
	doc += "motion.camera()\n"
	doc += "  按下相机键\n\n"
	doc += "motion.keyAction(code: number)\n"
	doc += "  按键动作\n\n"
	...
	doc += "  返回相对路径对应的绝对路径\n\n"
	doc += "files.listDir(path: string) -> table\n"
	...
	doc += "  获取指定坐标的像素颜色\n\n"
	doc += "images.captureScreen(x1: number, y1: number, x2: number, y2: number) -> userdata\n"
	doc += "  截取屏幕\n\n"
	...
	doc += "  查找多点颜色\n\n"
	doc += "images.readFromPath(path: string) -> userdata\n"
	doc += "  从路径读取图片\n\n"
	doc += "images.readFromUrl(url: string) -> userdata\n"
	doc += "  从URL读取图片\n\n"
	doc += "images.readFromBase64(base64Str: string) -> userdata\n"
	doc += "  从Base64读取图片\n\n"
	doc += "images.readFromBytes(bytes: string) -> userdata\n"
	doc += "  从字节数组读取图片\n\n"
	doc += "images.save(img: userdata, path: string, quality: number) -> boolean\n"
	doc += "  保存图片\n\n"
	doc += "images.encodeToBase64(img: userdata, format: string, quality: number) -> string\n"
	doc += "  编码为Base64\n\n"
	doc += "images.encodeToBytes(img: userdata, format: string, quality: number) -> string\n"
	doc += "  编码为字节数组\n\n"
	doc += "images.clip(img: userdata, x1: number, y1: number, x2: number, y2: number) -> userdata\n"
	doc += "  裁剪图片\n\n"
	doc += "images.resize(img: userdata, width: number, height: number) -> userdata\n"
	doc += "  调整图片大小\n\n"
	doc += "images.rotate(img: userdata, degree: number) -> userdata\n"
	doc += "  旋转图片\n\n"
	doc += "images.grayscale(img: userdata) -> userdata\n"
	doc += "  灰度化\n\n"
	doc += "images.applyThreshold(img: userdata, threshold: number, maxVal: number, typ: string) -> userdata\n"
	doc += "  应用阈值\n\n"
	doc += "images.applyAdaptiveThreshold(img: userdata, maxValue: number, adaptiveMethod: string, thresholdType: string, blockSize: number, C: number) -> userdata\n"
	doc += "  应用自适应阈值\n\n"
	doc += "images.applyBinarization(img: userdata, threshold: number) -> userdata\n"
	doc += "  二值化\n\n"
	...
	doc += "  判断键是否存在\n\n"
	doc += "storages.getAll(table: string) -> table\n"
	doc += "  获取所有键值对\n\n"
	...
	doc += "-- 网络请求 (https)\n"
	doc += "--------------------------------------------------\n"
	doc += "https.get(url: string, timeout: number) -> number, string\n"
	doc += "  发送GET请求\n\n"
	doc += "https.postMultipart(url: string, fileName: string, fileData: string) -> number, string\n"
	...
	doc += "-- 文字识别 (ppocr)\n"
	doc += "--------------------------------------------------\n"
	doc += "ppocr.ocr(x1: number, y1: number, x2: number, y2: number, colorStr: string) -> table\n"
	doc += "  识别屏幕文字\n\n"
	doc += "ppocr.ocrFromBase64(b64: string, colorStr: string) -> table\n"
	doc += "  识别Base64图片文字\n\n"
	doc += "ppocr.ocrFromPath(path: string, colorStr: string) -> table\n"
	doc += "  识别文件图片文字\n\n"
