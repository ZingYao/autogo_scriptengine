...
func (m *DeviceModule) Register(engine model.Engine) error {
	...
	deviceObj.RawSetString("getNotification", state.NewFunction(func(L *lua.LState) int {
		notifications := device.GetNotification()
		tbl := L.NewTable()
		for i, notification := range notifications {
			notificationTbl := L.NewTable()
			notificationTbl.RawSetString("Id", lua.LString(notification.Id))
			notificationTbl.RawSetString("PackageName", lua.LString(notification.PackageName))
			notificationTbl.RawSetString("Title", lua.LString(notification.Title))
			notificationTbl.RawSetString("Text", lua.LString(notification.Text))
			notificationTbl.RawSetString("Tag", lua.LString(notification.Tag))
			tbl.RawSetInt(i+1, notificationTbl)
		}
		L.Push(tbl)
		return 1
	}))
	...
	deviceObj.RawSetString("setDisplayPower", state.NewFunction(func(L *lua.LState) int {
		on := L.CheckBool(1)
	...
	engine.RegisterMethod("device.getNotification", "获取当前所有通知消息", device.GetNotification, true)
	...
	engine.RegisterMethod("device.setDisplayPower", "设置屏幕电源模式", func(on bool) { device.SetDisplayPower(on) }, true)
	engine.RegisterMethod("device.reboot", "重启设备", device.Reboot, true)
