...
// InstallVM 将调试命中函数注入到 go-lua-vm 状态机。
func (d *Debugger) InstallVM(state *glua.State) {
	if !d.Enabled() || state == nil {
		return
	}
	_ = glua.Register(state, hitFunction, func(args ...glua.Value) ([]glua.Value, error) {
		file := ""
		line := 0
		if len(args) > 0 {
			file = args[0].String
		...
		if len(args) > 1 {
			...
			case gruntime.KindInteger:
				line = int(args[1].Integer)
			case gruntime.KindNumber:
				line = int(args[1].Number)
			}
		}
		if err := d.Hit(file, line); err != nil {
			return nil, err
		}
		return nil, nil
	})
...
func (d *Debugger) Hit(file string, line int) error {
	if !d.Enabled() {
		return nil
	}
	...
	if d.isStopped() {
		...
		return fmt.Errorf("stopped by debugger")
	}
