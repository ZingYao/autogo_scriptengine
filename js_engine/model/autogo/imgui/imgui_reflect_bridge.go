...
func registerImguiReflectBridgeJS(engine model.Engine, imguiObj *goja.Object) {
	vm := engine.GetVM()
	for _, entry := range imguiReflectFuncs {
		if existing := imguiObj.Get(entry.name); existing == nil || goja.IsUndefined(existing) {
			imguiObj.Set(entry.name, bindImguiJSFunc(vm, entry.fn))
		}
	}
	for _, entry := range imguiReflectGenericFromCFuncs {
		if existing := imguiObj.Get(entry.name); existing == nil || goja.IsUndefined(existing) {
			imguiObj.Set(entry.name, bindImguiJSFunc(vm, reflect.ValueOf(entry.fn)))
		}
	}
	for _, entry := range imguiReflectGenericFromCFuncs {
...
func bindImguiJSFunc(vm *goja.Runtime, fn reflect.Value) func(goja.FunctionCall) goja.Value {
	fnType := fn.Type()
	return func(call goja.FunctionCall) (result goja.Value) {
		...
		args := buildImguiJSArgs(vm, call, fnType)
		if fnType.IsVariadic() {
			return pushImguiJSResults(vm, fn.CallSlice(args))
		}
		return pushImguiJSResults(vm, fn.Call(args))
	}
}

func bindImguiJSMethod(vm *goja.Runtime, value reflect.Value, methodName string) func(goja.FunctionCall) goja.Value {
	return func(call goja.FunctionCall) (result goja.Value) {
		defer func() {
			if recover() != nil {
				result = goja.Null()
		...
		args := buildImguiJSArgs(vm, call, methodType)
		if methodType.IsVariadic() {
			return pushImguiJSResults(vm, method.CallSlice(args))
		}
		return pushImguiJSResults(vm, method.Call(args))
	}
}

func buildImguiJSArgs(vm *goja.Runtime, call goja.FunctionCall, fnType reflect.Type) []reflect.Value {
	if !fnType.IsVariadic() {
		args := make([]reflect.Value, fnType.NumIn())
		for i := 0; i < fnType.NumIn(); i++ {
			args[i] = imguiJSToReflect(vm, call.Argument(i), fnType.In(i))
		}
		return args
	}
	...
	variadic := reflect.MakeSlice(variadicType, 0, imguiMaxInt(len(call.Arguments)-fixedCount, 0))
	for i := fixedCount; i < len(call.Arguments); i++ {
		variadic = reflect.Append(variadic, imguiJSToReflect(vm, call.Argument(i), variadicType.Elem()))
	}
	args[fnType.NumIn()-1] = variadic
	return args
}

func imguiJSToReflect(vm *goja.Runtime, value goja.Value, target reflect.Type) reflect.Value {
	if target.Kind() == reflect.Interface {
		return reflect.ValueOf(value.Export())
	}
		...
		if fn, ok := goja.AssertFunction(value); ok {
			return reflect.MakeFunc(target, func(args []reflect.Value) []reflect.Value {
				...
				for _, arg := range args {
					jsArgs = append(jsArgs, imguiReflectToJS(vm, arg))
				}
				...
				for i := 0; i < target.NumOut(); i++ {
					...
					results[i] = imguiJSToReflect(vm, result, target.Out(i))
				}
				return results
			})
		}
	...
	if target.Kind() == reflect.Ptr {
		if target.Elem().Kind() == reflect.Slice {
			if target.Elem().Elem().Kind() != reflect.Uint8 {
				...
				ptr.Elem().Set(imguiJSToReflect(vm, value, target.Elem()))
				return ptr
			}
			bytes := imguiJSValueToBytes(value)
			ptr := reflect.New(target.Elem())
			ptr.Elem().Set(reflect.ValueOf(bytes))
			return ptr
		}
		if unwrapped, ok := unwrapImguiJSGoValue(value); ok {
		...
		if target.Elem().Kind() == reflect.Struct {
			ptr := reflect.New(target.Elem())
			fillImguiJSStruct(vm, ptr.Elem(), value)
			return ptr
		}
		return reflect.Zero(target)
	}
	...
	switch target.Kind() {
	case reflect.Bool:
		return reflect.ValueOf(value.ToBoolean()).Convert(target)
	case reflect.String:
		return reflect.ValueOf(value.String()).Convert(target)
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		return reflect.ValueOf(value.ToInteger()).Convert(target)
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uintptr:
		return reflect.ValueOf(uint64(value.ToInteger())).Convert(target)
	case reflect.Float32, reflect.Float64:
		return reflect.ValueOf(value.ToFloat()).Convert(target)
	case reflect.Slice:
		if target.Elem().Kind() == reflect.Uint8 {
			return reflect.ValueOf(imguiJSValueToBytes(value)).Convert(target)
		}
		...
		result := reflect.MakeSlice(target, 0, length)
		for i := 0; i < length; i++ {
			result = reflect.Append(result, imguiJSToReflect(vm, obj.Get(imguiStrconvItoa(i)), target.Elem()))
		}
		return result
	case reflect.Array:
		obj := value.ToObject(nil)
		...
		for i := 0; i < target.Len(); i++ {
			result.Index(i).Set(imguiJSToReflect(vm, obj.Get(imguiStrconvItoa(i)), target.Elem()))
		}
		return result
	case reflect.Map:
		obj := value.ToObject(nil)
		...
		for _, key := range obj.Keys() {
			...
			if keyValue.Type().ConvertibleTo(target.Key()) {
				keyValue = keyValue.Convert(target.Key())
			} else {
				keyValue = reflect.Zero(target.Key())
			}
			result.SetMapIndex(keyValue, imguiJSToReflect(vm, obj.Get(key), target.Elem()))
		}
		return result
	case reflect.Struct:
		result := reflect.New(target).Elem()
		fillImguiJSStruct(vm, result, value)
		return result
	}
	return reflect.Zero(target)
}

func fillImguiJSStruct(vm *goja.Runtime, result reflect.Value, value goja.Value) {
	obj := value.ToObject(nil)
	if obj == nil {
		return
	}
	...
	for i := 0; i < result.NumField(); i++ {
		...
		fieldValue := obj.Get(fieldInfo.Name)
		if goja.IsUndefined(fieldValue) || goja.IsNull(fieldValue) {
			fieldValue = obj.Get(imguiLowerFirst(fieldInfo.Name))
		}
		if goja.IsUndefined(fieldValue) || goja.IsNull(fieldValue) {
			continue
		}
		field.Set(imguiJSToReflect(vm, fieldValue, field.Type()))
	}
}
...
func unwrapImguiJSGoValue(value goja.Value) (reflect.Value, bool) {
	if obj := value.ToObject(nil); obj != nil {
		goValue := obj.Get("__go")
		if goValue != nil && !goja.IsUndefined(goValue) && !goja.IsNull(goValue) {
			return reflect.ValueOf(goValue.Export()), true
		}
	}
	exported := value.Export()
	if exported != nil {
		return reflect.ValueOf(exported), true
	}
	return reflect.Value{}, false
}

func imguiJSValueToBytes(value goja.Value) []byte {
	if bytes, ok := value.Export().([]byte); ok {
		return bytes
	}
	if text, ok := value.Export().(string); ok {
		return []byte(text)
	}
	arr := value.ToObject(nil)
	if arr == nil {
		return nil
	}
	...
	for i := 0; i < length; i++ {
		bytes[i] = byte(arr.Get(imguiStrconvItoa(i)).ToInteger())
	}
	return bytes
}

func pushImguiJSResults(vm *goja.Runtime, results []reflect.Value) goja.Value {
	if len(results) == 0 {
		return goja.Undefined()
	}
	if len(results) == 1 {
		return imguiReflectToJS(vm, results[0])
	}
	obj := vm.NewObject()
	for i, result := range results {
		obj.Set("r"+imguiStrconvItoa(i), imguiReflectToJS(vm, result))
	}
	return obj
}

func imguiReflectToJS(vm *goja.Runtime, value reflect.Value) goja.Value {
	if !value.IsValid() {
		return goja.Null()
	}
	...
	switch value.Kind() {
	case reflect.Bool:
		return vm.ToValue(value.Bool())
	case reflect.String:
		return vm.ToValue(value.String())
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		return vm.ToValue(value.Int())
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uintptr:
		return vm.ToValue(value.Uint())
	case reflect.Float32, reflect.Float64:
		return vm.ToValue(value.Float())
	case reflect.Slice, reflect.Array:
		if value.Type().Elem().Kind() == reflect.Uint8 {
			return vm.ToValue(value.Bytes())
		}
		arr := vm.NewArray()
		for i := 0; i < value.Len(); i++ {
			arr.Set(imguiStrconvItoa(i), imguiReflectToJS(vm, value.Index(i)))
		}
		return arr
	case reflect.Map:
		obj := vm.NewObject()
		iter := value.MapRange()
		for iter.Next() {
			obj.Set(iter.Key().String(), imguiReflectToJS(vm, iter.Value()))
		}
		return obj
	case reflect.Func:
		return vm.ToValue(func(call goja.FunctionCall) goja.Value {
			...
			args := buildImguiJSArgs(vm, call, fnType)
			if fnType.IsVariadic() {
				return pushImguiJSResults(vm, value.CallSlice(args))
			}
			return pushImguiJSResults(vm, value.Call(args))
		})
	}
	return wrapImguiJSObject(vm, value)
}

func wrapImguiJSObject(vm *goja.Runtime, value reflect.Value) goja.Value {
	if value.Kind() == reflect.Ptr && value.IsNil() {
		return goja.Null()
	}
	obj := vm.NewObject()
	obj.Set("__go", value.Interface())
	typ := value.Type()
	if typ.Kind() == reflect.Ptr {
		typ = typ.Elem()
	}
	for _, entry := range imguiReflectMethods[typ.Name()] {
		if existing := obj.Get(entry.name); existing == nil || goja.IsUndefined(existing) {
			obj.Set(entry.name, bindImguiJSMethod(vm, value, entry.goMethod))
		}
	}
	return obj
}
