...
func (m *OpencvModule) Register(engine model.Engine) error {
	...
	for _, entry := range openCVFuncs {
		opencvObj.Set(entry.name, bindOpenCVJSFunc(vm, entry.fn))
	}
...
func bindOpenCVJSFunc(vm *goja.Runtime, fn reflect.Value) func(goja.FunctionCall) goja.Value {
	fnType := fn.Type()
	return func(call goja.FunctionCall) (result goja.Value) {
		...
		args := buildOpenCVJSArgs(call, fnType)
		if fnType.IsVariadic() {
			return pushOpenCVJSResults(vm, fn.CallSlice(args))
		}
		return pushOpenCVJSResults(vm, fn.Call(args))
	}
}

func bindOpenCVJSMethod(vm *goja.Runtime, value reflect.Value, methodName string) func(goja.FunctionCall) goja.Value {
	return func(call goja.FunctionCall) (result goja.Value) {
		defer func() {
			if recover() != nil {
				result = goja.Null()
		...
		args := buildOpenCVJSArgs(call, methodType)
		if methodType.IsVariadic() {
			return pushOpenCVJSResults(vm, method.CallSlice(args))
		}
		return pushOpenCVJSResults(vm, method.Call(args))
	}
}

func buildOpenCVJSArgs(call goja.FunctionCall, fnType reflect.Type) []reflect.Value {
	if !fnType.IsVariadic() {
		args := make([]reflect.Value, fnType.NumIn())
		for i := 0; i < fnType.NumIn(); i++ {
			args[i] = jsToReflect(call.Argument(i), fnType.In(i))
		}
		return args
	}
	...
	variadic := reflect.MakeSlice(variadicType, 0, maxInt(len(call.Arguments)-fixedCount, 0))
	for i := fixedCount; i < len(call.Arguments); i++ {
		variadic = reflect.Append(variadic, jsToReflect(call.Argument(i), variadicType.Elem()))
	}
	args[fnType.NumIn()-1] = variadic
	return args
}

func jsToReflect(value goja.Value, target reflect.Type) reflect.Value {
	...
	if target.Kind() == reflect.Ptr {
		if target.Elem().Kind() == reflect.Slice {
			if target.Elem().Elem().Kind() != reflect.Uint8 {
				...
				ptr.Elem().Set(jsToReflect(value, target.Elem()))
				return ptr
			}
			bytes := jsValueToBytes(value)
			ptr := reflect.New(target.Elem())
			ptr.Elem().Set(reflect.ValueOf(bytes))
			return ptr
		}
		if unwrapped, ok := unwrapJSGoValue(value); ok {
	...
	case reflect.Slice:
		if target.Elem().Kind() == reflect.Uint8 {
			return reflect.ValueOf(jsValueToBytes(value)).Convert(target)
		}
		...
		for i := 0; i < length; i++ {
			result = reflect.Append(result, jsToReflect(obj.Get(strconvItoa(i)), target.Elem()))
		}
		return result
	case reflect.Array:
		obj := value.ToObject(nil)
		...
		for i := 0; i < target.Len(); i++ {
			result.Index(i).Set(jsToReflect(obj.Get(strconvItoa(i)), target.Elem()))
		}
		return result
	case reflect.Map:
		obj := value.ToObject(nil)
		...
		for _, key := range obj.Keys() {
			...
			result.SetMapIndex(keyValue, jsToReflect(obj.Get(key), target.Elem()))
		}
		return result
	case reflect.Struct:
		result := reflect.New(target).Elem()
		fillJSStruct(result, value)
		return result
	}
	return reflect.Zero(target)
}

func fillJSStruct(result reflect.Value, value goja.Value) {
	obj := value.ToObject(nil)
	if obj == nil {
		return
	}
	...
	for i := 0; i < result.NumField(); i++ {
		...
		if goja.IsUndefined(fieldValue) || goja.IsNull(fieldValue) {
			fieldValue = obj.Get(lowerFirst(fieldInfo.Name))
		}
		if goja.IsUndefined(fieldValue) || goja.IsNull(fieldValue) {
			continue
		}
		field.Set(jsToReflect(fieldValue, field.Type()))
	}
}
...
func unwrapJSGoValue(value goja.Value) (reflect.Value, bool) {
...
func jsValueToBytes(value goja.Value) []byte {
	...
	lengthValue := arr.Get("length")
	length := int(lengthValue.ToInteger())
	bytes := make([]byte, length)
	for i := 0; i < length; i++ {
		bytes[i] = byte(arr.Get(strconvItoa(i)).ToInteger())
	}
	return bytes
}

func pushOpenCVJSResults(vm *goja.Runtime, results []reflect.Value) goja.Value {
	if len(results) == 0 {
		return goja.Undefined()
	}
	if len(results) == 1 {
		return reflectToJS(vm, results[0])
	}
	obj := vm.NewObject()
	for i, result := range results {
		obj.Set("r"+strconvItoa(i), reflectToJS(vm, result))
	}
	if len(results) == 2 && isReflectInt(results[0]) && isReflectInt(results[1]) {
		obj.Set("x", reflectToJS(vm, results[0]))
		obj.Set("y", reflectToJS(vm, results[1]))
	}
	return obj
}

func reflectToJS(vm *goja.Runtime, value reflect.Value) goja.Value {
	if !value.IsValid() {
		return goja.Null()
	}
	...
	case reflect.Slice, reflect.Array:
		...
		for i := 0; i < value.Len(); i++ {
			arr.Set(strconvItoa(i), reflectToJS(vm, value.Index(i)))
		}
		return arr
	case reflect.Map:
		obj := vm.NewObject()
		iter := value.MapRange()
		for iter.Next() {
			obj.Set(iter.Key().String(), reflectToJS(vm, iter.Value()))
		}
		return obj
	}
	return wrapOpenCVJSObject(vm, value)
}

func wrapOpenCVJSObject(vm *goja.Runtime, value reflect.Value) goja.Value {
	if value.Kind() == reflect.Ptr && value.IsNil() {
		return goja.Null()
	}
	obj := vm.NewObject()
	...
	for _, entry := range openCVMethods[typ.Name()] {
		obj.Set(entry.name, bindOpenCVJSMethod(vm, value, entry.goMethod))
	}
	return obj
}
